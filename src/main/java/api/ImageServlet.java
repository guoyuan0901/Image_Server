...
public class ImageServlet extends HttpServlet {
   ...
   // doGet方法根据请求生成响应
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        //查看所有图片属性和查看指定图片属性,通过是否 URL 中带有 imageId 参数来进行区分.
    ...
    private void selectAll(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        Gson gson = new GsonBuilder().create();
        //jsonData 就是一个 json 格式的字符串了, 与之前约定的格式是一样的.
        String jsonData = gson.toJson(images);
        // 这个方法的核心：json 帮我们自动完成了大量的格式转换工作
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        // 1. 获取图片的属性信息, 并且存入数据库
           ...
           // a) 需要创建一个 factory 对象 和 upload 对象, 为了获取到图片属性   固定的逻辑
        FileItemFactory factory = new DiskFileItemFactory();
        ServletFileUpload upload = new ServletFileUpload(factory);

           //b) 通过 upload 对象进一步解析请求(解析HTTP请求中奇怪的 body 中的内容)
                 //一个FileItem 就代表一个上传的文件对象.
        List<FileItem> items = null;
        try {
            items = upload.parseRequest(req);//解析请求
        } catch (FileUploadException e) {
            // 出现异常说明解析出错!
            e.printStackTrace();
            // 告诉客户端出现的具体的错误(以json进行返回）
            resp.setContentType("application/json; charset=utf-8");
        ...
        image.setUploadTime(simpleDateFormat.format(new Date()));//Date()获取当前日期
        // simpleDateFormat为了格式化日期
        image.setContentType(fileItem.getContentType());
        image.setMd5(DigestUtils.md5Hex(fileItem.get()));//计算md5
        image.setPath("./image/" + image.getMd5());
        //  ./表示当前目录  使用MD5来作为磁盘存储路径
        ...
        if (existImage == null) {
            File file = new File(image.getPath());//新建一个文件
            try {
                fileItem.write(file);
            } catch (Exception e) {
                e.printStackTrace();
